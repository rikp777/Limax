<template>
    <div :class="{'notification-container': true, 'notification-container-empty': items.length === 0}">
        <transition-group name="ntf" tag="div" made="out">
            <div
                v-for="item in items"
                :key="item.id" :class="'notification notification-'+item.options.type"
                @click="remove(item.id)">
                <h4 class="title" v-if="item.title">{{item.title}}</h4>
                <div class="message" v-if="item.message" v-html="item.message"></div>
            </div>
        </transition-group>
    </div>
</template>
<script>
    export default {
        ...
        methods: {
            ...
            addItem(type, title, message, option){
                let defaultOptions = {
                    type: type,
            ...
            removeItem(uuid){
                this.items = Object.assign([], this.items.filter(x => x.id !== uuid))
            },
            removeAll(){
                this.items = []
            }
